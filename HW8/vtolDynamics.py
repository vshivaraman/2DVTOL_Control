...
class vtolDynamics():
    ...
    def rk4(self,state,rotForce):
        ...
        #print("k1 = ",k1)
        k2 = self.func(state+step/2*k1,rotForce)
        k3 = self.func(state+step/2*k2,rotForce)
        k4 = self.func(state+step*k3,rotForce)
        self.state += step/6*(k1+2*k2+2*k3+k4)
...
#vtol = vtolDynamics()
#state = vtol.updated_state([10.2,10.2])
#state = vtol.updated_state([10.2,10.2])
#print(state)
