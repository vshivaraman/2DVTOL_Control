...
class vtolDynamics():
    ...
    def func(self,state,rotForce):
        ...
        f1 = -(rotForceLeft+rotForceRight)*sin(theta) - self.dragCoef*Z_
        f2 = (rotForceLeft+rotForceRight)*cos(theta) - (self.mass_centerpod + 2*self.mass_rotor)*self.g
        f3 = (rotForceRight-rotForceLeft)*self.rod_length
        ...
        lhs = np.matrix([[m1,0,0],[0,m2,0],[0,0,m3]])
    ...
    def updated_state(self,rotForce):
        ...
        """ Change the values in the list passed to the rk4 function
            for changing the force by the left and right rotor respectively"""
...
class vtolDynamics():
    ...
    def __init__(self):
        self.state = np.array([[5.],[0.0],[0.*pi/180],[0.],[0.],[0.]])
        self.mass_centerpod = 1.5
        self.inertia_centerpod = 0.006
        self.mass_rotor = 0.27
        self.rod_length = 0.28
    ...
    def func(self,state,rotForce):
        ...
        f1 = rotForceLeft*sin(theta*pi/180)+rotForceRight*sin(theta*pi/180)-self.dragCoef*Z_ # Should add drag force
        f2 = rotForceLeft*cos(theta*pi/180)+rotForceRight*cos(theta*pi/180) - (self.mass_centerpod+2*self.mass_rotor)*self.g # Should add the constant term (m1)
        f3 = (rotForceLeft-rotForceRight)*self.rod_length # Drag Force torque to be added ??
        rhs = np.array([f1,f2,f3])
        
        m1 = self.mass_centerpod+2*self.mass_rotor
        m2 = self.mass_centerpod+2*self.mass_rotor
        m3 = self.inertia_centerpod+4*self.mass_rotor*self.rod_length**2
